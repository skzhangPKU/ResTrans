...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, root, datatxt, transform=None, target_transform=None):
        ...
        imgs = []
        for line in fh:
            # line = line.strip().replace('/mnt/data/zhangsk/LearnData','E:\\OSLAB\\LearnDependency\\data_bank').replace('/','\\')
            line = line.strip()
            items = line.split('|')
            if items[4] == 'ia':
                label = 0
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            # raw_img = self.target_transform(self.transform(raw_img).resize((224,224), Image.ANTIALIAS))
            # tmp_transform = self.transform(element_img).resize((224, 224), Image.ANTIALIAS)
            element_img = self.target_transform(self.transform(element_img).resize((224, 224), Image.ANTIALIAS))
        # return raw_img,element_img,all_ids, ele_ids, label
...
def data_batch():
    ...
    for batch in train_loader:
        ...
        # data, target = Variable(raw_img, volatile=True), Variable(element_img)
    print('hha')
