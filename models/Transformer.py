...
class Generator(nn.Module):
	...
	def __init__(self, d_model, class_num):
		...
		self.ip1 = nn.Linear(49 * self.class_num, 100)
		self.ip2 = nn.Linear(100,self.class_num)


	def forward(self, x):
		proj_x = self.preluip1(self.batchnorm1(self.proj(x)))
		proj_x = proj_x.view(-1,x.shape[1]*self.class_num)
		ip1 = self.preluip1(self.batchnorm2(self.ip1(proj_x)))
		ip2 = self.ip2(ip1)
...
class Transformer(nn.Module):
	def make_model(self, class_num, N=6,
			   d_model=512, d_ff=2048, h=8, dropout=0.1):
		"Helper: Construct a model from hyperparameters."
		# maximum number of elements in one UI
		max_ele_number = 100
		c = copy.deepcopy
	...
	def __init__(self, args):
		...
		self.model = self.make_model(self.class_num, self.num_layers, self.embed_size,
	...
	def forward(self, att_feats, att_masks = None):
		...
		out = self.model(att_feats, att_masks) #att_masks:None
